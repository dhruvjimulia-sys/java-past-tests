...
public class GenericList<T> implements GenericListInterface<T> {
  ...
  // post: Adds the given item at the given position in the list. It throws an
  // exception if the position is out of bounds.
  public void add(int pos, T item) throws ListIndexOutOfBoundsException {
    if (pos == 1) {
      head = new ListNode<>(item, head);
      numItems++;
    } else if (pos > 1 && pos <= numItems + 1) {
      final ListNode<T> prev = find(pos - 1);
      final ListNode<T> curr = prev.getNext();
      prev.setNext(new ListNode<>(item, curr));
      numItems++;
    } else {
      throw new ListIndexOutOfBoundsException("position out of range in add method of list");
    }
  }

  // post: Removes the item at the given position in the list. It throws an
  // exceptions if the position is out of bounds.
  public void remove(int pos) throws ListIndexOutOfBoundsException {
    if (pos >= 1 && pos <= numItems) {
      ...
      throw new ListIndexOutOfBoundsException("position out of range in remove method of a list");
    }
  } // end remove
