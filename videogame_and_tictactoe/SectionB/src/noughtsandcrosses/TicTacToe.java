...
public class TicTacToe {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("****************************");
    System.out.println(
        "Do you want to play as (" + String.valueOf(Board.CROSS) + ") and go first? [yes]: ");
    ...
    if (user.nextLine().trim().equals("yes")) {
      userMark = Board.CROSS;
    } else {
      userMark = Board.NOUGHT;
    }

    if (userMark == Board.CROSS) {
      computerMark = Board.NOUGHT;
    } else {
      computerMark = Board.CROSS;
    }
    ...
    while (!board.isFinished()) {
      ...
      if (board.getTurn() == userMark) {
        System.out.print(
            "Please enter an empty position in [1..9] for your next placement of ("
  ...
  */
  private static int computerOptimalMove(Board board) {
    ...
    final Random optimalMoveSelector = new Random(1234567);
