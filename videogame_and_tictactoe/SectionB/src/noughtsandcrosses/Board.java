...
public class Board {
  ...
  // post: Returns the mark ('X' or 'O') of the winner player if a winning condition exists in the
  // board,
  ...
  public void setMark(int pos, char mark) throws GameException {
    ...
    if (pos < 1 || pos > 9) {
      throw new GameException("attempted to set mark in a wrong position: " + pos);
    }

    if (mark != NOUGHT && mark != CROSS) {
      throw new GameException("attempted to set an invalid mark: " + String.valueOf(mark));
    }
    ...
    if (grid[row][col] != EMPTY) {
      throw new GameException("attempted to set mark on a non-empty position: " + pos);
    } else {
      grid[row][col] = mark;
      numOfMarks++;
    }
