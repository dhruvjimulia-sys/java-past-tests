...
public class GameTree implements GameTreeInterface {
  ...
  private int sizeTree(GameTreeNode node) {
    int sizeOfChildTrees = 0;
    for (GameTreeNode child : node.getChildren()) {
      sizeOfChildTrees += sizeTree(child);
    }
    return node.getChildren().size() + sizeOfChildTrees;
  }
  ...
  private void expandTree(GameTreeNode node) {
    ...
    if (!board.isFinished()) {
      final char nextPlayer = board.getTurn();
      for (int i = 1; i < 10; i++) {
        if (board.getMark(i) == Board.EMPTY) {
          final Board newBoard = board.makeCopy();
          newBoard.setMark(i, nextPlayer);
          newBoard.setLastMarkPosition(i);
          final GameTreeNode newTreeNode = new GameTreeNode(newBoard);
          node.getChildren().add(1, newTreeNode);
          expandTree(newTreeNode);
        }
      }
    }
  }
