...
public class Thread {
  ...
  /**
   * Determines whether the thread is enabled in the context of a store
   *
   * @param store variable store representing the program state
  ...
  /**
   * Causes the thread to make an execution step in the context of the given store
   *
   * @param store variable store representing the program state
   */
  public void step(Store store) {
    if (isTerminated()) {
      throw new UnsupportedOperationException("Terminated thread cannot execute");
    }
