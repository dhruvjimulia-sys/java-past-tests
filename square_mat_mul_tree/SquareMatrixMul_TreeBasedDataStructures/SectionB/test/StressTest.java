...
import collections.FineCompactWordTree;
...
public class StressTest {
  ...
  private static ArrayList<String> generateRandomStrings(int numTestStrings, int maxStringLength) {
    ...
    IntStream.range(0, numTestStrings)
        .forEach(
            w -> {
  ...
  public void stressTestWithManyOperations() {
    ...
    final CompactWordsSet wordsSet = new FineCompactWordTree();
          ...
          Thread newThread =
              new Thread(
                  () -> {
                    ...
                    wordsToBeInsertedByThreadI.forEach(
                        w -> {
                          try {
                            wordsSet.add(w);
                            wordsSet.contains(w);
  ...
  public void performanceTest() {
    ...
    IntStream.range(1, NUM_THREADS + 1).forEach(nt -> performanceTestWithNThreads(nt));
  }
          ...
          Thread newThread =
              new Thread(
                  () -> {
                    ...
                    final int indexToExclusive =
                        Math.min((t + 1) * (segmentLength + 1), manyValidStrings.size());

                    IntStream.range(indexFromInclusive, indexToExclusive)
                        .forEach(
                            i -> {
                              try {
  ...
  private void checkResult(CompactWordsSet wordsSet) {
    ...
    assertEquals(
        "The CompactWordSet contains duplicates",
        uniqueCollectedWords.size(),
        uniqueCollectedWordsAsSet.size());
    assertEquals(
        "The CompactWordSet contains an unexpected set of words",
        uniqueTestWords,
